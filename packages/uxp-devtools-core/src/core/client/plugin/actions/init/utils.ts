...
function getConflictingFilesList(root: string): string[] {
  const unsafeFiles = [
    'package.json',
    'manifest.json',
    'yarn.lock',
...
function sanityCheckPluginDirectory(root: string): void {
...
function checkIfOnline(): Promise<boolean> {
    ...
    dns.lookup('registry.yarnpkg.com', (err) => {
      if (err != null) {
        ...
        if (proxy) {
          ...
          dns.lookup(parsedUrl.hostname, (proxyErr) => {
