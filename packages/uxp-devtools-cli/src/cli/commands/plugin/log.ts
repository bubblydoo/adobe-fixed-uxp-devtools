...
async function handlePluginLogsCommand(this: unknown, args: ArgumentsCamelCase<LogCommandArgs>): Promise<void> {
  // set the forConsole to true - so that we show only the Console Tab in the CDT Inspect for Logs.
  args.forConsole = true;
  // Call the debug handler with the same `this` context
  const handler = debugCommand.handler as (this: unknown, args: ArgumentsCamelCase<LogCommandArgs>) => Promise<void>;
  await handler.call(this, args);
}

const logOptions: Record<string, Options> = {
  apps: {
...
const logCommand: CommandModule<object, LogCommandArgs> = {
  command: 'logs',
  describe: 'Show Plugin Logs Window',
