...
interface CommandContext {
  app: {
    server: {
      ...
      disableDevTools: (options: { port?: number }) => Promise<boolean>;
      isServiceRunning: () => Promise<RunningStatus>;
    };
  };
}

function waitForKey(keyNames: string[]): Promise<boolean> {
  ...
  process.stdin.on('keypress', (_c: string, k: { name: string }) => {
    if (process.stdin.isTTY) {
...
function promptUserForTermsAccept(enable: boolean, runningStatus: RunningStatus = { success: false }): Promise<boolean> {
  if (enable) {
...
function handleEnableCommand(this: CommandContext): Promise<void> {
...
const enableCommand: CommandModule = {
  command: 'enable',
  describe: 'Configures the UXP Developer service to permit loading and debugging of plugins in development.',
  handler: handleEnableCommand,
};

function handleDisableCommand(this: CommandContext): void {
...
const disableCommand: CommandModule = {
  command: 'disable',
  describe: 'Prevents plugins in development from being loaded or debugged.',
